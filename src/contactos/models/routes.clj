...
(defn build-grid-require
  ...
  [table]
  (let [data (str "   [contactos.handlers.admin." table ".controller :as " table "-controller]")]
    data))

(defn build-reporte-require
  "Genera archivos requeridos para un reporte para un controller"
  [table]
  (let [data (str "   [contactos.handlers.reports." table ".controller :as " table "-report]")]
    data))

(defn build-require
  "Genera archivos requeridos para un dashboard para una tabla"
  [table]
  (let [data (str "   [contactos.handlers." table ".controller :as " table "-dashboard]")]
    data))
