(ns contactos.layout
  (:require
   ...
   [contactos.models.crud :refer [config]]
   [contactos.models.util :refer [user-level user-name]]
   [contactos.menu :as menu]
...
(defn menus-private [request]
  (let [{:keys [nav-links dropdowns]} (contactos.menu/get-menu-config)]
...
(defn app-scripts [request]
  (list
   ...
   (theme-js)
   ;; fk-dependent.js: FK-dependent selects & create modal (vanilla JS)
