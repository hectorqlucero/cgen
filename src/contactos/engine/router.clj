(ns contactos.engine.router
  (:require
   [clojure.string :as str]
   [compojure.core :refer [defroutes GET POST context]]
   [ring.util.response :refer [redirect]]
   [hiccup2.core :refer [html]]
   [contactos.engine.config :as config]
   [contactos.engine.query :as query]
   [contactos.engine.crud :as crud]
   [contactos.engine.render :as render]
   [contactos.tabgrid.core :as tabgrid]
   [contactos.models.util :refer [get-session-id user-level]]
                  ...
                  edited-id (or (get params :edited_id) (get params "edited_id")
                   ...
                   url (or (let [base (if (and return-url active-tab)
                           ...
                           (str "/admin/" entity-name
                                (when selected-id
                                  (str "/" selected-id))))]
               (println "[DEBUG] handle-save redirect: entity=" entity-name
                        "return-url=" return-url "active-tab=" active-tab
                        "edited-id=" edited-id "url=" url)
               (redirect url))
            {:status 400
