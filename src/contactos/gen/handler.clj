...
(defn- gen-controller
  [handler-name]
  (str "(ns contactos.handlers." handler-name ".controller\n"
       "  (:require\n"
       "   [contactos.handlers." handler-name ".model :as model]\n"
       "   [contactos.handlers." handler-name ".view :as view]\n"
       "   [contactos.layout :refer [application]]\n"
...
(defn- gen-model
  [handler-name]
  (str "(ns contactos.handlers." handler-name ".model\n"
       "  (:require\n"
       "   [contactos.models.crud :refer [Query]]))\n"))

(defn- gen-view
  [handler-name]
  (str "(ns contactos.handlers." handler-name ".view)\n"))
...
(def proutes-path "src/contactos/routes/proutes.clj")

(defn- require-entry
  "Build the require vector string for a handler controller."
  [handler-name]
  (str "[contactos.handlers." handler-name ".controller :as " handler-name "]"))
...
(defn- remove-handler!
  ...
  (let [dir-name (ns-name->path handler-name)
        base-dir (str "src/contactos/handlers/" dir-name)]
