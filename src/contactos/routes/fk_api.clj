(ns contactos.routes.fk-api
  (:require
   [compojure.core :refer [defroutes GET POST]]
   [contactos.engine.config :as config]
   [contactos.engine.crud :as crud]
   [contactos.models.crud :as model-crud]
   [contactos.models.util :refer [json-response]]
      ...
      (try
        (let [entity-config (config/get-entity-config entity)
              ...
              form-fields (map #(select-keys % [:id :label :type :required? :placeholder
                               ...
                               fields)
              rendered (let [render-fn #'contactos.engine.render/render-field]
